...
def submit_html_patient_form(context):
    ...
    context.currentCount = len(requests.get(
        "http://localhost:8000/patients/view-all-patients").json())
...
def check_patient_information_added(context):
    ok_(context.currentCount<len(requests.get(
        "http://localhost:8000/patients/view-all-patients").json()),"Patient Registration Failed")
    
@given("Request for Patient reports for PatientID")
def fetch_patient_reports_from_browser(context):
    context.driver = webdriver.Chrome()
    context.driver.get("http://localhost:8000/reports/view/5")
...
def add_patient_report(context):
    ...
    context.currentCount = len(requests.get(
        "http://localhost:8000/reports/view-all").json())
...
def check_report_information_added(context):
    ok_(context.currentCount<len(requests.get(
        "http://localhost:8000/reports/view-all").json()),"Report Registration Failed")
